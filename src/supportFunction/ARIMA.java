...
public class ARIMA {
	...
	public int[] predictValue(int p, int q) {
		int[] predictions = new int[5]; // 创建用于存储预测值的数组
		double[] stdoriginalData = this.preDealDif();
		int n = stdoriginalData.length;
		double[] err = new double[q + 1];
		Random random = new Random();

		for (int predictionIndex = 0; predictionIndex < 5; predictionIndex++) {
			int predict = 0;
			...
			predictions[predictionIndex] = predict; // 将预测值存储在数组中
		}

		return predictions; // 返回包含多个预测值的数组
	}
